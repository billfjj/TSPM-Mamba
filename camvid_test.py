...
def label2rgb(mask):
    ...
    mask_rgb[np.all(mask_convert == 2, axis=0)] = [192, 192, 128]
    mask_rgb[np.all(mask_convert == 3, axis=0)] = [128, 64, 128]
    mask_rgb[np.all(mask_convert == 4, axis=0)] = [0, 0, 192]
    mask_rgb[np.all(mask_convert == 5, axis=0)] = [128, 128, 0]
    mask_rgb[np.all(mask_convert == 6, axis=0)] = [192, 128, 128]
    mask_rgb[np.all(mask_convert == 7, axis=0)] = [64, 64, 128]
    mask_rgb[np.all(mask_convert == 8, axis=0)] = [64, 0, 128]
    mask_rgb[np.all(mask_convert == 9, axis=0)] = [64, 64, 0]
    mask_rgb[np.all(mask_convert == 10, axis=0)] = [0, 128, 192]
    return mask_rgb
    ...
    with torch.no_grad():
        ...
        for input in tqdm(test_loader):
            ...
            for i in range(raw_predictions.shape[0]):
                ...
                mask_name = image_ids
                ori_img =  'data/CamVid/CamVid_RGB/' + image_ids + '.png'
                if args.val:
    ...
    if args.val:
        ...
        for class_name, class_iou, class_f1, class_ac in zip(config.classes, iou_per_class, f1_per_class, ac_per_class):
            print('mF1_{}:{}, IOU_{}:{}, AP_{}:{}'.format(class_name, class_f1,
                                                          ...
                                                          class_name, class_ac))
        print('mF1:{}, mIOU:{}, OA:{}, mAP:{}'.format(np.nanmean(f1_per_class),
