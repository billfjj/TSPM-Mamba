...
from ossfm_mamba_model.classification.models.vmamba_8d import VSSBlock
from ossfm_mamba_model.models.LearnableFilters import LearnableFilters
from ossfm_mamba_model.models.ResT import ResT
...
def rest_small(pretrained=True, weight_path='pretrain_weights/rest_small.pth',  **kwargs):
    model = ResT(embed_dims=[64, 128, 256, 512], num_heads=[1, 2, 4, 8], mlp_ratios=[4, 4, 4, 4], qkv_bias=True,
...
def rest_base(pretrained=True, weight_path='pretrain_weights/rest_base.pth',  **kwargs):
...
class MambaSegDecoder(nn.Module):
    def __init__(
        ...
        depths = [2, 2, 2, 2]
        # depths = [8, 8, 8, 8]
        dpr = [x.item() for x in torch.linspace(drop_path_rate, 0, (n_stages_encoder - 1) * depths[0])]
        ...
        for s in range(1, n_stages_encoder):
            ...
            stages.append(VSSLayer(
                ...
                drop_path=dpr[sum(depths[:s - 1]):sum(depths[:s + (depths[0] - 2)])],
    ...
    def forward(self, skips, h, w):
        ...
        if self.training:
            ...
            for s in range(len(self.stages)):
                ...
                x = self.dropout1(x)  # 减少过拟合
                if s == (len(self.stages) - 1):
                    x = self.dropout2(x)     # 减少过拟合
                    seg_out = self.seg(x)
                else:
...
class TSP2Mamba(nn.Module):
    def __init__(self,
                 pretrained,
                 input_size=(1024, 1024),
                 extract_model_name='rest_small',
                 embed_dim=64,
                 num_classes=6,
                 **kwargs
                 ):
        ...
        self.encoder, embed_dim = globals()[extract_model_name]()
        encoder_channels = [embed_dim, embed_dim * 2, embed_dim * 4, embed_dim * 8]
        scale_factors = [4, 8, 16, 32]
        self.lbf_list = []
        for i, dim in enumerate(encoder_channels):
            self.lbf_list.append(
                LearnableFilters(dim, input_size[0] // scale_factors[i], input_size[0] // scale_factors[i])
            )
    ...
    def forward(self, x):
        ...
        for i, output in enumerate(outputs):
            lbf_output = self.lbf_list[i](output)
            out_new.append(outputs[i] + lbf_output)

        if self.training:
            x, lsm = self.decoder(out_new, h, w)
            return x, lsm

        else:
            x = self.decoder(out_new, h, w)
            return x
