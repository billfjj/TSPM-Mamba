...
from tsp2mamba_model.datasets.swjtu_dataset import *
...
weights_name = "TSP2Mamba-swjtu-"+str(image_size)+"-e"+str(max_epoch) + "-" + now.strftime("%Y%m%d%H%M")
weights_path = "model_weights/swjtu/{}".format(weights_name)
test_weights_name = "last-v2"
test_weights_path = 'model_weights/swjtu/TSP2Mamba-swjtu-512-e200-202411071428/model-epoch=191-val_mIoU=0.67.ckpt'
log_name = 'swjtu/{}'.format(weights_name)
monitor = 'val_mIoU'
monitor_mode = 'max'
save_top_k = 1
save_last = True
...
#  define the network
net = TSP2Mamba(pretrained=pretrained_ckpt_path,
...
def train_aug(img, mask):
    crop_aug = Compose([Resize(size=INPUT_IMG_SIZE),
...
train_dataset = SWJTUTrainDataset(
    transform=train_aug,
    txt_dir='train_images.txt',
    img_size=INPUT_IMG_SIZE
)

val_dataset = SWJTUTrainDataset(
    txt_dir='val_images.txt',
    mosaic_ratio=0.0,
    transform=val_aug,
    img_size=INPUT_IMG_SIZE
)

# test_dataset = SWJTUTestDataset(img_size=TEST_IMG_SIZE)
